...
import (
	...
	testing_consts "github.com/khulnasoft/khulnasoft-operator/test/consts"
...
func CreateKhulnasoftDatabasePassword(namespace string) *corev1.Secret {
...
func CreatePvc(namespace string) *corev1.PersistentVolume {
	return &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "khulnasoftdb-pv",
			Namespace: namespace,
			Labels: map[string]string{
				"app": "khulnasoft-database",
			},
		},
...
func CreateNode(name string) *corev1.Node {
	return &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:   name,
			Labels: map[string]string{"for": "khulnasoft-testing-node", "app": "hostpath-provisioner"},
		},
...
//
//	_, err = exec.Command("bash", "-c", "PATH=~:/usr/local/bin/:$PATH && kind delete cluster").Output()
//	"kubectl get pods -n ${namespace} -l app.kubernetes.io/instance=khulnasoft-${chart} -o jsonpath='{.items[*].status.containerStatuses[0].ready}'"
//}
